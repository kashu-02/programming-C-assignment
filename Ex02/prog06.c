...
int main(){
  ...
  if((fp = fopen("score_data.txt", "r")) == NULL){
    fprintf(stderr, "ファイル読み込みエラー\n");
    exit(1);
  }
  while (fscanf(fp, "%d %d", &student_id[num_student], &abs_score[num_student]) != EOF) num_student++;
  fclose(fp);
  ...
  while(1){
    printf("表示したい評価を大文字で入れてください(A-D,F): ");
...
void grade_search(char c){
  int i;
  printf("%cの一覧\n", c);
  for(i = 0; i < num_student; i++){
    if(rel_grade[i] == (int) c){
      printf("%d %d %c\n", student_id[i], rel_score[i], rel_grade[i]);
    }
  }
}
